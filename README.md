...
覆盖正常登记与逐期收款、金额合计不符、期数重复、超额收款、草稿/已过期拒绝、空计划、逾期三类期数区分、跨合同操作拒绝等场景；边界用例包括：应收日期当天/前一天/后一天的逾期判定（当天不算逾期）、第三位小数为 4/5 的舍入、收款恰好等于剩余判结清、分期各自进位后合计变化的拒绝。每个用例自建并清理数据，连续运行结果一致。
